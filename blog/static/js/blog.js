...
var HIDDENFIELD = "?mainurl=" + window.CURRENT_URL;
...
function initBlogSGang(u, p, currentUrl) {
  ...
  window.HIDDENFIELD = "?mainurl=" + window.CURRENT_URL;
  XMLHTTPURL_LOGIN = "user/login/blog" + HIDDENFIELD;
    ...
    if (this.readyState == 4 && this.status == 200) {
      ...
      (function () {
        ...
        (function () {
          ...
          let s = { 'csrfmiddlewaretoken': checkUserToken, 'user': u, 'password': p, 'currentUrl': HIDDENFIELD };
          const request =
          {
            method: 'POST',
            credentials: 'include',
            headers: {
              'X-CSRFToken': checkUserToken, 'Content-Type':
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      if (comments_json.length > 0) {
        ...
        mess.push(new Post("post", "tinkyblink", "Commenta Per Primo", ".....", "", BASE_URL + "static/images/user-secret-solid.gif", "0"))
...
function htmlIframeWidthHeight() {
  ...
  window.top.postMessage({ "height": height, "base": width }, '*');
