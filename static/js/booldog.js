...
let f = function () {
    ...
    window.addEventListener("message", function (event) {
        if (!(event.origin.includes('https://127.0.0.1:8000'))) {
            ...
            if (event.data.reload === true) {
                document.getElementById('booldogFrame').src = "https://127.0.0.1:8000/booldog?mainurl=" + this.location.href.toString() + "&user=" + user + "&password=" + password;
    ...
    if (height == "") {
        ...
        iframe.setAttribute('src', 'https://127.0.0.1:8000/booldog?mainurl=' + mainurl + "&user=" + user + "&password=" + password);
